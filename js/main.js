(() => {
	...
	const addMenuBackground = () => {
        ...
        if(pageWidth > mobileWidth) {
            boddyOffset > 0 ? navigation.classList.add("tph-nav-fixed") : navigation.classList.remove("tph-nav-fixed");
        }
	}

	const reorderResponsiveMenu = () => {
        const pageWidth = window.innerWidth;
        const navContainer = document.querySelector("header nav .tph-container");
        const navigation = document.querySelector("header nav .tph-navigation");
        const mobileNavigation = document.querySelector("body > .tph-navigation");
	...
	const mobileMenuToggle = () => {
        const menuToggle = document.querySelector(".tph-nav-toggle");

        menuToggle.addEventListener("click", () =>{
        	const mobileNavigation = document.querySelector("body > .tph-navigation");

        	mobileNavigation.classList.toggle("tph-navigation-opened");
        })
	}

	const onNavItemClick = () => {
		const navItemList = document.querySelectorAll(".tph-section-link");
	...
	const onTestimonialChange = () => {
        ...
        const prevArrow = document.querySelector("#tph-testimonials-prev");
        const nextArrow = document.querySelector("#tph-testimonials-next");
        const testimonials = document.querySelector(".tph-testimonials ul");
	...
	const onGalleryImageClick = () => {
        const galleryImageList = document.querySelectorAll("#tph-gallery li");
	...
	const galleryImageOpen = image => {
		...
		const openedImage = `<div class='tph-backdrop'><img src='${imageSrc}' alt='' />
		                    <span class="tph-backdrop-close">X</span></div>`;
	...
	const galleryImageClose = () => {
		const closeButton = document.querySelector(".tph-backdrop-close");

		closeButton.addEventListener("click", () => {
			const backdrop = document.querySelector(".tph-backdrop");
			backdrop.remove();
		})
	}
